...
func _init() -> void:
	...
	if not T.require_true(self, fetcher != null, "Failed to instantiate Fetcher.gd"):
		return
	if not T.require_true(self, fetcher.has_method("fetch_delete"), "Fetcher must expose fetch_delete()"):
		return

	var response: Variant = fetcher.call("fetch_delete", base_url + "/delete")
	if not T.require_true(self, response != null, "Fetcher.fetch_delete() must return a response object"):
		return
	if not T.require_eq(self, response.call("get_status"), 200, "Unexpected DELETE /delete status"):
		return
	var body := String(response.call("get_text"))
	if not T.require_true(self, body.contains('"deleted": true') or body.contains('"deleted":true'), "Response body must confirm deletion"):
		return
