...
| REQ-0001-004 | `PRD-0001` | `v1-static-fetcher-and-session-parity.md` | `fetchers-static` | sync/async methods/headers/status PASS 2026-03-08 | in_progress |
| REQ-0001-005 | `PRD-0001` | `v1-static-fetcher-and-session-parity.md` | `fetchers-static` | session defaults/cookies/async-session/rotator/proxy/custom-strategy/utils PASS 2026-03-08 | in_progress |
...
- 当前仓库已具备 parser 核心等价层，以及静态 fetcher 的 GET / POST / PUT / DELETE / response-headers / headers / params / cookies / 404 / timeout 最小实现，以及 `AsyncFetcher` / `AsyncFetcherSession` 的 GET / POST / PUT / DELETE、response-headers / 404 / timeout、cookie 持久化与默认值行为；并已打通 `ProxyRotator` 的 cyclic / custom strategy、自省接口、`cyclic_rotation()` / `is_proxy_error()` 工具函数与真实代理接线；browser、spider、tooling 仍待推进。
