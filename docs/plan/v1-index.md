...
- 当前仓库已具备 parser 核心等价层，以及静态 fetcher 的 GET / POST / PUT / DELETE / response-headers / headers / params / cookies / 404 / timeout 最小实现，以及 `AsyncFetcher` / `AsyncFetcherSession` 的 GET / POST / PUT / DELETE、cookie 持久化与默认值行为；并已打通 `ProxyRotator` 的 cyclic / custom strategy、自省接口、`cyclic_rotation()` / `is_proxy_error()` 工具函数与真实代理接线；browser、spider、tooling 仍待推进。
