...
- 已验证：`powershell -File .\scripts\run_godot_tests.ps1 -Suite fetchers-static` 返回 `PASS`，当前已覆盖本地 fixture server 下的 `fetch_get()` / `fetch_post()` / `fetch_put()` / `fetch_delete()`、`AsyncFetcher.fetch_get()` / `fetch_post()` / `fetch_put()` / `fetch_delete()`、`AsyncFetcherSession.fetch_get()` / `fetch_post()` / `fetch_put()` / `fetch_delete()`、同步/异步响应头读取、`headers` / `params`、显式 `cookies`、`FetcherSession` 与 `AsyncFetcherSession` 的 cookie 持久化与默认配置、`retries` / `retry_delay_sec`、`ProxyRotator` 的 cyclic / custom strategy、自省接口、`cyclic_rotation()` / `is_proxy_error()` 工具能力、真实代理链路下的 per-request override / cyclic rotation，以及同步/异步 `404` / `timeout` 行为。
- 进行中：更完整错误分类仍待补齐。
